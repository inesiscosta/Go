...
def obtain_adjacent_intersections(intersection: 'tuple[str,int]', ultima_intersection: 'tuple[str,int]') -> 'tuple[tuple[str,int], ...]':
    """
    ...
    :type intersection: tuple[str,int]
    :param ultima_intersection: The intersection in the upper right corner of the Go board.
    :type ultima_intersection: tuple(str,int)
    ...
    for x, y in [(0, -1), (-1, 0), (1, 0), (0, 1)]: # Adjacent Coordinate Vectors
        try:
            ...
            if ord(obtain_col(new_intersection)) <= ord(obtain_col(ultima_intersection)) \
                and obtain_row(new_intersection) <= obtain_row(ultima_intersection):
    ...
    while not valid_turn:
        ...
        if move == 'P':
            ...
            try:
                ...
                print("Invalid Move.")
                # If the move is illegal ask for a move again.
    return True
